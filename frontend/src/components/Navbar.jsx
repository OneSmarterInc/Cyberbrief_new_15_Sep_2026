...
export default function Navbar({ 
  ...
  const [isBuffering, setIsBuffering] = useState(false); // NEW: Tracks backend generation time
  ...
  const readSummary = () => {
    // If it is currently speaking or trying to load, stop it
    ...
    // Set UI to loading instantly
    setIsBuffering(true);
    
    window.audioPlayer = new Audio(audioUrl);
    
    window.audioPlayer.onplay = () => { 
      setIsBuffering(false); // Audio arrived, stop buffering
      setSpeaking(true); 
      setProgress(0); 
    };
  ...
  return (
    <header className="aggregate-clone">
      <style>{`
        ...
        /* Spin Animation for the Buffering Hourglass */
        ...
        <div className="aggregate-wrap aggregate-briefing-inner">
          ...
          <div className="aggregate-player-container">
            <div className="aggregate-player">
              ...
              >
                {/* DYNAMIC ICON: Hourglass if loading, Square if playing, Triangle if paused */}
